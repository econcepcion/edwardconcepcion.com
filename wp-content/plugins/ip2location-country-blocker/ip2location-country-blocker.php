...
class IP2LocationCountryBlocker
{
	...
	public function settings_page()
	{
		...
		$nonce = wp_create_nonce('update-database');

		echo '<input type="hidden" id="update_nonce" value="' . $nonce . '">';
	}
	...
	public function admin_page()
	{
		...
		wp_enqueue_script('iplcb-chosen-js', 'https://cdnjs.cloudflare.com/ajax/libs/chosen/1.7.0/chosen.jquery.min.js', [], null, true);
		wp_enqueue_script('iplcb-chart-js', 'https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.8.0/Chart.min.js', [], null, true);
		wp_enqueue_script('iplcb-tagsinput-js', plugins_url('/assets/js/jquery.tagsinput.min.js', __FILE__), [], null, true);

		wp_enqueue_style('iplcb-chosen-css', esc_url_raw('https://cdnjs.cloudflare.com/ajax/libs/chosen/1.7.0/chosen.min.css'), [], null);
		wp_enqueue_style('iplcb-tagsinput-css', esc_url_raw('https://cdnjs.cloudflare.com/ajax/libs/jquery-tagsinput/1.3.6/jquery.tagsinput.min.css'), [], null);
	...
	public function plugin_enqueues($hook)
	{
		...
		switch ($hook) {
			...
			case 'country-blocker_page_ip2location-country-blocker-statistics':
				wp_enqueue_script('iplcb-chart-js', plugins_url('/assets/js/Chart.min.js', __FILE__), ['jquery'], null, true);
			...
			foreach ($_POST as $key => $values) {
				if (is_array($values)) {
					foreach ($values as $index => $value) {
						$values[$index] = sanitize_text_field($value);
					}

					$_POST[$key] = $values;
				} else {
					$_POST[$key] = sanitize_text_field($values);
				}
			}
		}
	}
